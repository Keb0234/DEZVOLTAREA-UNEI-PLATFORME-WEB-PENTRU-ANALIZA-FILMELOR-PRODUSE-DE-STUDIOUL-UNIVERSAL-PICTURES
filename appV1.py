...
def manipulare_date():
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Show Movies From DB", "Insert Movie", "Delete Movie", "Edit Movie", "Dataset Infomation"])

    with tab2:
        st.header("Insert movie")
        create_movie_title = st.text_input("Movie Title", key="create_movie_title")
        create_movie_release_date = st.date_input("Movie Release Date", key="create_movie_release_date")
        converted_release_date = datetime.combine(create_movie_release_date, time())
        create_movie_runtime = st.number_input("Movie Runtime", key="create_movie_runtime", step=1)
        create_movie_budget = st.number_input("Budget (USD)", key="create_movie_budget",step=1)
        create_movie_box_office = st.number_input("Movie Box Office (USD)", key="create_movie_box_office",step=1)
        create_movie_genres = st.text_input("Genres", key="create_movie_genres")
        create_movie_cast = st.text_input("Top Billed Cast", key="create_movie_cast")
        create_movie_age_rating = st.selectbox("Age Rating", ("G", "PG", "PG-13", "R", "NC-17"),index = None, key="create_movie_age_rating")
        create_movie_box_office_domestic = st.number_input("Movie Domestic Box Office (USD)", key="create_movie_box_office_dom",step=1)
        create_movie_box_office_international = st.number_input("Movie International Box Office (USD)", key="create_movie_box_office_int",step=1)
        
        franchise_list = collection.distinct("franchise")
        franchise_list.append("Other")
        create_movie_franchise = st.selectbox("Movie Franchise", franchise_list, index = 0, key="create_movie_franchise")
        selected_movie_franchise = " "
        if create_movie_franchise == "Other":
            create_new_movie_franchise = st.text_input("Insert New Franchise", key="create_new_movie_franchse")
    ...
    with tab1:
        ...
        with st.expander("Filter Movies"):
                col_filter_genre, col_filter_franchise, col_filter_ratings, col_filter_cast = st.columns(4)
                with col_filter_genre:
                    select_filter_genre = st.multiselect("Select Genre(s)", list_genres, key="select_genre_filter")
                with col_filter_franchise:
                    select_filter_franchise = st.multiselect("Select Franchise(s)", franchise_list, key="select_franchise_filter")
                with col_filter_ratings:
                    select_filter_ratings = st.multiselect("Select MPAA Rating(s)", list_ratings, key="select_rating_filter")
                with col_filter_cast:
                    select_filter_cast = st.multiselect("Select Actor(s)", list_cast, key="select_cast_filter")
                    
                col_filter_budget, col_filter_box_total = st.columns(2)
                with col_filter_budget:
                    select_filter_budget = st.slider("Select the budget", min_db_budget, max_db_budget, initial_range_budget, step=50000000 ,format="$%d",key="filter_budget")
                    budget_min_ales, budget_max_ales = select_filter_budget
                with col_filter_box_total:
                    select_filter_box_total = st.slider("Select the total box office", min_db_box_office_total, max_db_box_office_total, initial_range_box_office_total, step=50000000 ,format="$%d", key="range_box")
                    min_db_box_office_total, max_db_box_office_total = select_filter_box_total
                
                with st.expander("Filter by domestic and international box office"):
                    col_filter_box_dom, col_filter_box_int = st.columns(2)
                    with col_filter_box_dom:
                        select_filter_box_dom = st.slider("Select the domestic box office", min_db_box_office_dom, max_db_box_office_dom, initial_range_box_office_dom, step=50000000 ,format="$%d", key="range_box_dom")
                        min_db_box_office_dom, max_db_box_office_dom = select_filter_box_dom
                    with col_filter_box_int:
                        select_filter_box_int = st.slider("Select the international box office", min_db_box_office_int, max_db_box_office_int, initial_range_box_office_int, step=50000000 ,format="$%d", key="range_box_int")
                ...
                with col_filter_release_date:
                    select_filter_date = st.slider("Select the Release Date", min_release_date, max_release_date, initial_range_release_date,key="filter_release_date")
                    min_release_date, max_release_date = select_filter_date
                with col_filter_runtime:
                    select_filter_runtime = st.slider("Select the Runtime", min_db_runtime, max_db_runtime, initial_range_runtime,format="%d minute", key="filter_runtime")
        ...
        for movie in movies:
            ...
            with st.container(border=True):
                ...
                with col2:
                    ...
                    st.text_input("Franchise", value=movie.get("franchise"), key=f"franchise_{m_id}_{st.session_state['db_version']}")

                    c_date, c_run = st.columns(2)
                    with c_date:
                        st.date_input("Release Date", value=movie.get("releaseDate"), key=f"date_{m_id}_{st.session_state['db_version']}")
                    with c_run:
                        st.number_input("Runtime (minutes)", value=movie.get("runTime"), key=f"run_{m_id}_{st.session_state['db_version']}")
                    ...
                    with c_box_total:
                        if (movie.get("boxOffice")):
                            ...
                            st.text_input("Box Office - Total", value="Released exclusively on streaming - No Box Office Data", key=f"box_2{m_id}_{st.session_state['db_version']}")
                    with c_budget:
                        if (movie.get("budget")):
                            st.number_input("Budget", value=movie.get("budget"), key=f"budget_{m_id}_{st.session_state['db_version']}")
                        else:
                            st.text_input("Budget", value="No Budget Available", key=f"budget_2{m_id}_{st.session_state['db_version']}")
                    ...
                    if (movie.get("boxOffice")):
                        ...
                        with c_box_off_dom:
                            if (movie.get("boxOffice_domestic")):
                                ...
                                st.text_input("Box Office - Domestic", value="Released exclusively on streaming in The US",key=f"box_dom{m_id}_{st.session_state['db_version']}")
                        with c_box_off_int:
                            if (movie.get("boxOffice_international")):
                                ...
                                st.text_input("Box Office - International", value="Released exclusively on streaming outside The US", key=f"box_int_2{m_id}_{st.session_state['db_version']}")
                    genres = movie.get("genres", [])
                    if genres:
                        st.text_input("Genres", f"{' | '.join(genres)}", key=f"genres_{m_id}_{st.session_state['db_version']}")
                    cast = movie.get("top_cast", [])
                    if cast:
                        st.text_input("Cast", f"{' | '.join(cast)}", key=f"cast_{m_id}_{st.session_state['db_version']}")
                    # st.text_input("MPAA Rating", value=movie.get("age_rating"), key=f"age_rating_{m_id}")
                    mpaa_rating_logo(movie.get("age_rating"))
            
    with tab3:
        st.header("Delete Movie From DB")
        movies = get_movies_list()
        movie_to_delete = st.selectbox("Select movie to delete", options=movies,index=None)
        if st.button("Delete movie"):
            query = {"movieTitle": movie_to_delete}
            result = collection.delete_one(query)
            st.cache_data.clear()
            if result.deleted_count > 0:
                st.success(f"Movie {movie_to_delete} was deleted")
            else:
                st.warning("error deleting")

    with tab4:
        st.header("Edit Movie From DB")
        movies = get_movies_list()
        movie_to_edit = st.selectbox("Select movie to edit", options=movies,index=None)
        if movie_to_edit:
            ...
            with st.form("edit_form"):
                edit_movie_title = st.text_input("Movie Title", value= editable_movie.get("movieTitle"), key=f"edit_title_{movie_to_edit}_{st.session_state['db_version']}")
                edit_movie_franchise = st.selectbox("Franchise", options=franchise_list, index=current_franchise_index, key=f"edit_franchise_{movie_to_edit}_{st.session_state['db_version']}")
                create_new_movie_franchise = st.text_input("Insert New Franchise", key=f"create_new_movie_franchse_edit_{movie_to_edit}_{st.session_state['db_version']}")
                edit_movie_release_date = st.date_input("Movie Release Date",value= editable_movie.get("releaseDate"),key=f"edit_date_{movie_to_edit}_{st.session_state['db_version']}")
                edit_release_date = datetime.combine(edit_movie_release_date, time())
                edit_movie_runtime = st.number_input("Movie Runtime", step=1, value= editable_movie.get("runTime"),key=f"edit_time_{movie_to_edit}_{st.session_state['db_version']}")
                edit_movie_box_office = st.number_input("Movie Box Office - Total", value= editable_movie.get("boxOffice"), step=1,key=f"edit_boxT_{movie_to_edit}_{st.session_state['db_version']}")
                edit_movie_box_office_dom = st.number_input("Movie Box Office - Domestic", value= editable_movie.get("boxOffice_domestic"), step=1, key=f"edit_boxD_{movie_to_edit}_{st.session_state['db_version']}")
                edit_movie_box_office_int = st.number_input("Movie Box Office - International", value= editable_movie.get("boxOffice_international"), step=1, key=f"edit_BoxI_{movie_to_edit}_{st.session_state['db_version']}")
                edit_movie_budget = st.number_input("Movie Budget", value= editable_movie.get("budget"), step=1,key=f"edit_budget_{movie_to_edit}_{st.session_state['db_version']}")
                current_genres = ", ".join(editable_movie.get("genres", []))
                edit_movie_genres = st.text_input("Genres", value=current_genres, key=f"edit_genre_{movie_to_edit}_{st.session_state['db_version']}")
                current_cast = ", ".join(editable_movie.get("top_cast", []))
                edit_movie_cast = st.text_input("Top Cast", value=current_cast, key=f"edit_cast_{movie_to_edit}_{st.session_state['db_version']}")
                ...
                edit_movie_age_rating = st.selectbox("Age Rating", options=mpaa_ratings, index=current_rating_index,key=f"edit_rating_{movie_to_edit}_{st.session_state['db_version']}")

                if st.form_submit_button("Edit Movie"):
                    selected_movie_franchise = " "
                    ...
                    if result.modified_count > 0:
                        st.success(f"Movie {movie_to_edit} was edited")
                        st.session_state['db_version'] += 1
                        st.rerun()
                    else:
    ...
    with tab5:
        ...
        if "df_shared" in st.session_state:
            ...
            columns_to_download = st.multiselect("Select Columns to Download", df.columns)
            df_to_download = df[columns_to_download]
            csv = df_to_download.to_csv(index=False).encode('utf-8')
            #date_of_dl = date.today()
            st.download_button("Download CSV", csv, f"Date_Universal_Pictures_{date.today()}.csv", "secondary")

            st.divider()
            st.header("Informatii dataset")
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Number of Films in database:", len(df))
            with col2:
                avg_run = np.mean(df['runTime']) if 'runTime' in df.columns else 0
                st.metric("Avg Runtime Per Movie", f"{avg_run:.1f} min")
            with col3:
                avg_box = np.mean(df['boxOffice']) if 'boxOffice' in df.columns else 0
                st.metric("Average Box Office Per Movie", f"${avg_box:,.0f} (USD)")
            with col4:
                avg_budget = np.mean(df['budget']) if 'budget' in df.columns else 0
                st.metric("Average Budget Per Movie", f"${avg_budget:,.0f} (USD)")
            ...
            titlu_ptr_bar = st.multiselect("Select Movie for Financial Comparison",list_titles, key="char")
            df_film_selectat = df[df["movieTitle"].isin(titlu_ptr_bar)]
            fig_box1 = px.bar(
                df_film_selectat,
                title=f"Financial Comparison For: {titlu_ptr_bar}",
                x = "movieTitle",
...
def analiza_date_partea_1():
    ...
    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    if not numeric_cols:
        st.error("Nu exista coloane numerice in dataset!")
        return
    ...
    cat_col = st.selectbox("Select Category for Financial Breakdown:", cat_cols, key="cat_col2")
    ...
    fig_box2 = px.bar(
        df_grouped,
        title=f"Financial Breakdown For: {cat_col}",
        x = cat_col,
        y = ["budget","boxOffice","boxOffice_domestic","boxOffice_international"],
        labels={
            "value" : "Millions (USD)",
    ...
    def iqr_outlier_mask(df_: pd.DataFrame, col: str, k: float = 1.5):
    ...
    for col in numeric_cols:
        ...
        n_out = int(mask.sum())
        pct_out = (n_out / len(df) * 100) if len(df) else 0.0
    ...
    st.header("Altair")
    heatmap = alt.Chart(df,title = 'Distributie de filme per an per luna').mark_rect().encode(
    x=alt.X('month_name:O', title='Release Month', sort=['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']),
    y=alt.Y('year:O', title='Release Year'),
    ...
    chart_master = alt.Chart(df_explodat, title = "Genre").mark_bar().encode(
        x=alt.X('genres:N', title='Movie Genre', sort='-y'),
        y=alt.Y('sum(boxOffice):Q', title='Total Box Office (USD)', axis=alt.Axis(format='$~s')),
    ...
    chart_detail = alt.Chart(df_explodat, title = "Box Office Per Genre Per Movie").mark_bar(invalid=None).encode(
        x=alt.X('movieTitle:N', title='Movie in Genre', sort='-y'),
        y=alt.Y('boxOffice:Q', title='Box Office', axis=alt.Axis(format='$~s')),
        color=alt.Color('age_rating:N', title='MPAA Rating'),
    ...
    scatter = base.mark_circle(size=100).encode(color=alt.condition(brush, 'movieTitle:N', alt.value('lightgray'))).add_params(brush)
    ...
    st.altair_chart(quadrant_chart, use_container_width=True, legend=None)
...
def analiza_date_partea_2():
    ...
    for extra_col in ['genres', 'top_cast','year']:
        if extra_col in df.columns and extra_col not in cat_cols:
            cat_cols.append(extra_col)
